...
// @version      1.16
// @description  拦截舰C数据并推送到Supabase后端（建造资源消耗全栈版，修复speedchange分支匹配bug）
// @author       Qing-Feng
...
(function() {
    ...
    if (window.self !== window.top) {
        ...
        console.log('[script_1.16] 在iframe中运行，不创建UI面板');
    }
    ...
    function handleApiData(url, apiData, requestBody = null) {
        ...
        // 3. 建造全栈事件
        // 注意：必须先判 createship_speedchange，再判 createship
        // （createship_speedchange 的 URL 也包含 /createship 前缀，顺序反了会被吞）
    ...
    createPanel();
    addLog('<span style="color:#ffd700;">脚本已激活 script_1.16 @ ' + location.host + '，等待舰C请求...</span>');
