...
// @version      1.14
// @description  拦截舰C数据并推送到Supabase后端（建造资源消耗全栈版）
// @author       Qing-Feng
...
(function() {
    ...
    if (window.self !== window.top) {
        console.log('[script_1.14] 在iframe中运行，不创建UI面板');
    }
    ...
    function handleApiData(url, apiData, requestBody = null) {
        ...
        // 1. 原有分支：舰队/舰娘数据
        if (url.includes('/api_get_member/preset_deck')) return;
    ...
    // 4. 全局网络请求监听（MutationObserver 检测 iframe）
    ...
    createPanel();
    addLog('<span style="color:#ffd700;">脚本已激活 script_1.14 @ ' + location.host + '，等待舰C请求...</span>');
    addLog('<span style="color:#888;">已启用: XHR劫持 + fetch劫持 + 建造全栈拦截</span>');
